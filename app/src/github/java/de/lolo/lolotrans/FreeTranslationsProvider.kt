    ...
    override suspend fun translate(
        ...
        try {
            ...
            when (val code = conn.responseCode) {
                ...
                403 -> Result.failure(IllegalStateException("Zugriff verweigert (HTTP 403). API-Key ungueltig."))
                429 -> Result.failure(IllegalStateException("Zu viele Anfragen (HTTP 429). Bitte spaeter erneut versuchen."))
                500 -> Result.failure(IllegalStateException("Server-Fehler (HTTP 500). Der Server hat ein internes Problem."))
                else -> {
                    val errorBody = conn.errorStream?.let { readStream(it) } ?: ""
                    Result.failure(IllegalStateException("Uebersetzungsfehler (HTTP $code): $errorBody"))
                }
            }
        } catch (e: Exception) {
